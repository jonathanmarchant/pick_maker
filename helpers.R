...
#' jm_nfl_schedule(season = "2018")
jm_nfl_schedule <- function(season){
  ...
  # year > 1969
  season_numeric <- as.integer(season)
  season_start_yr <- as.character(season_numeric)
  season_end_yr <- as.character(season_numeric + 1)
  if(nchar(season_start_yr) > 4){
    season_dates <- season
  } else {
    season_dates <- glue::glue("{season_start_yr}0401-{season_end_yr}0331")
  }
